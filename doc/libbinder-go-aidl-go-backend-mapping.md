...
### 3.2 fixed-size array 规则

`T[N]` 固定映射为 `[N]T`，不退化成 `[]T`。

原因：

- 长度约束直接进入 Go 类型系统
- 生成 API 能在编译期表达固定长度语义
- runtime codec 内部可以临时转成 slice 写入，但公开 API 不丢失信息
...
| `in T arg` | 输入参数 `arg T` |
| `out T arg` | 返回值 `arg T` |
| `inout T arg` | 输入参数 `arg T` + 返回值 `argOut T` |
