...
JAVA_METADATA_SERVER_MAIN="com.wdsgyj.libbinder.aidltest.javaserver.MetadataServerMain"
JAVA_METADATA_CLIENT_MAIN="com.wdsgyj.libbinder.aidltest.javaclient.MetadataClientMain"

LISTENER_SERVICE_NAME="${LISTENER_SERVICE_NAME:-libbinder.go.aidltest.listener}"
METADATA_SERVICE_NAME="${METADATA_SERVICE_NAME:-libbinder.go.aidltest.metadata}"

REMOTE_LISTENER_GO_CLIENT="${REMOTE_DIR}/listener-go-client"
REMOTE_LISTENER_GO_SERVER="${REMOTE_DIR}/listener-go-server"
REMOTE_METADATA_GO_CLIENT="${REMOTE_DIR}/metadata-go-client"
REMOTE_METADATA_GO_SERVER="${REMOTE_DIR}/metadata-go-server"

REMOTE_LISTENER_JAVA_SERVER_LOG="${REMOTE_DIR}/listener-java-server.log"
REMOTE_LISTENER_JAVA_CLIENT_LOG="${REMOTE_DIR}/listener-java-client.log"
REMOTE_LISTENER_GO_SERVER_LOG="${REMOTE_DIR}/listener-go-server.log"
REMOTE_LISTENER_GO_CLIENT_LOG="${REMOTE_DIR}/listener-go-client.log"
REMOTE_METADATA_JAVA_SERVER_LOG="${REMOTE_DIR}/metadata-java-server.log"
REMOTE_METADATA_JAVA_CLIENT_LOG="${REMOTE_DIR}/metadata-java-client.log"
REMOTE_METADATA_GO_SERVER_LOG="${REMOTE_DIR}/metadata-go-server.log"
REMOTE_METADATA_GO_CLIENT_LOG="${REMOTE_DIR}/metadata-go-client.log"
...
log() {
  echo "[android-aidl-governance-cases] $*"
}

usage() {
  cat <<'EOF'
Usage:
  ./scripts/android-aidl-governance-cases.sh

Runs the governance Android emulator AIDL compatibility slice:
  1. Java server -> Go client (listener)
  2. Go server -> Java client (listener)
  3. Java server -> Go client (metadata)
  4. Go server -> Java client (metadata)
EOF
}
...
dump_case_logs() {
  ...
  for path in \
    ...
    "${REMOTE_LISTENER_GO_CLIENT_LOG}" \
    "${REMOTE_METADATA_JAVA_SERVER_LOG}" \
    "${REMOTE_METADATA_JAVA_CLIENT_LOG}" \
    "${REMOTE_METADATA_GO_SERVER_LOG}" \
    "${REMOTE_METADATA_GO_CLIENT_LOG}"; do
    echo "--- ${path} ---"
...
prepare_target() {
  ...
  emulator_log="$(mktemp "${TMPDIR:-/tmp}/libbinder-go-aidl-governance.XXXXXX")"
...
build_go_fixtures() {
  ...
  (
    ...
    env "${GO_ENV_ANDROID[@]}" go build -o /tmp/metadata-go-client ./tests/aidl/go/client/metadata
    env "${GO_ENV_ANDROID[@]}" go build -o /tmp/metadata-go-server ./tests/aidl/go/server/metadata
  )
}
...
push_go_fixtures() {
  ...
  "${ADB_BIN}" -s "${ANDROID_SERIAL}" push /tmp/metadata-go-client "${REMOTE_METADATA_GO_CLIENT}" >/dev/null
  "${ADB_BIN}" -s "${ANDROID_SERIAL}" push /tmp/metadata-go-server "${REMOTE_METADATA_GO_SERVER}" >/dev/null
  "${ADB_BIN}" -s "${ANDROID_SERIAL}" shell "chmod 755 $(android_shell_quote "${REMOTE_LISTENER_GO_CLIENT}") $(android_shell_quote "${REMOTE_LISTENER_GO_SERVER}") $(android_shell_quote "${REMOTE_METADATA_GO_CLIENT}") $(android_shell_quote "${REMOTE_METADATA_GO_SERVER}")" >/dev/null
}
...
start_go_listener_server() {
  local service_name="$1"
  local log_path="$2"
  start_background "${log_path}" "cd $(android_shell_quote "${REMOTE_DIR}") && exec ./$(basename "${REMOTE_LISTENER_GO_SERVER}") $(android_join_shell_words "-service" "${service_name}")"
}
...
run_go_listener_client() {
  local service_name="$1"
  local log_path="$2"
  run_foreground_to_log "${log_path}" "cd $(android_shell_quote "${REMOTE_DIR}") && exec ./$(basename "${REMOTE_LISTENER_GO_CLIENT}") $(android_join_shell_words "-service" "${service_name}")"
}

run_case_java_server_go_client_listener() {
  log "case: java_server_go_client listener"
  start_java_server "${JAVA_LISTENER_SERVER_MAIN}" "${LISTENER_SERVICE_NAME}" "_" "${REMOTE_LISTENER_JAVA_SERVER_LOG}"
  run_go_listener_client "${LISTENER_SERVICE_NAME}" "${REMOTE_LISTENER_GO_CLIENT_LOG}"
  stop_last_server
}

run_case_go_server_java_client_listener() {
  log "case: go_server_java_client listener"
  start_go_listener_server "${LISTENER_SERVICE_NAME}" "${REMOTE_LISTENER_GO_SERVER_LOG}"
  run_java_client "${JAVA_LISTENER_CLIENT_MAIN}" "${LISTENER_SERVICE_NAME}" "_" "${REMOTE_LISTENER_JAVA_CLIENT_LOG}"
  stop_last_server
}

run_case_java_server_go_client_metadata() {
  log "case: java_server_go_client metadata"
  start_java_server "${JAVA_METADATA_SERVER_MAIN}" "${METADATA_SERVICE_NAME}" "java" "${REMOTE_METADATA_JAVA_SERVER_LOG}"
  run_go_client "${REMOTE_METADATA_GO_CLIENT}" "${METADATA_SERVICE_NAME}" "java" "${REMOTE_METADATA_GO_CLIENT_LOG}"
  stop_last_server
}

run_case_go_server_java_client_metadata() {
  log "case: go_server_java_client metadata"
  start_go_server "${REMOTE_METADATA_GO_SERVER}" "${METADATA_SERVICE_NAME}" "go" "${REMOTE_METADATA_GO_SERVER_LOG}"
  run_java_client "${JAVA_METADATA_CLIENT_MAIN}" "${METADATA_SERVICE_NAME}" "go" "${REMOTE_METADATA_JAVA_CLIENT_LOG}"
  stop_last_server
}
...
main() {
  ...
  log "all governance AIDL cases passed on ${ANDROID_SERIAL}"
}
