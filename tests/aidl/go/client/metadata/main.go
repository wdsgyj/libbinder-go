...
func main() {
	...
	svc, err := shared.WaitIMetadataServiceService(ctx, conn.ServiceManager(), *serviceName)
	if err != nil {
		fatalf("wait service %q: %v", *serviceName, err)
	}
	if err := cases.VerifyMetadataService(ctx, svc, *expectPrefix); err != nil {
		fatalf("verify metadata service: %v", err)
	}
