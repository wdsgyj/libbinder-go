...
func main() {
	...
	svc, err := shared.WaitIAdvancedServiceService(ctx, conn.ServiceManager(), *serviceName)
	if err != nil {
		fatalf("wait service %q: %v", *serviceName, err)
	}
	if err := cases.VerifyAdvancedService(ctx, svc, *expectPrefix); err != nil {
		fatalf("verify advanced service: %v", err)
	}
